...
struct resolver_entity* resolver_follow_arrary_bracket(struct resolver_process* resolver, struct node* node, struct resolver_result* result);
...
struct resolver_entity* resolver_entity_clone(struct resolver_entity* entity)
{
    ...
    struct resolver_entity* cloned_entity = calloc(1, sizeof(struct resolver_entity));
    memcpy(cloned_entity, entity, sizeof(struct resolver_entity));
...
struct resolver_scope* resolver_proces_scope_current(struct resolver_process* process)
{
...
struct resolver_entity* resolver_create_new_entity_for_merged_array_brackets(struct resolver_result* result, struct resolver_process* process, struct node* node, struct node* array_index_node, int index, struct datatype* dtype, void* private, struct resolver_scope* scope)
{
    struct resolver_entity* entity = resolver_create_new_entity(result, RESOLVER_ENTITY_TYPE_ARRAY_BRACKET, private);
    if (!entity)
    {
...
struct resolver_entity* resolver_create_new_unary_indirection_entity(struct resolver_process* process, struct resolver_result* result, struct node* node, int indirection_depth)
{
    ...
    // Pointer cannot actually know the address it needs to point at compile time
    entity->flags = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_LEFT_ENTITY | RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTITY;
    entity->node = node;
...
struct resolver_entity* resolver_create_new_entity_for_var_node_custom_scope(struct resolver_process* process, struct node* var_node, void* private, struct resolver_scope* scope, int offset)
{
    ...
    struct resolver_entity* entity = resolver_create_new_entity(NULL, RESOLVER_ENTITY_TYPE_VARAIBLE, private);
...
{
    struct resolver_entity* entity = resolver_new_entity_for_var_node_no_push(process, var_node, private, offset,resolver_proces_scope_current(process));
    if (!entity)
    {
        return NULL;
    }
...
// creates entity based on node type
...
struct resolver_entity* resolver_register_function(struct resolver_process* process, struct node* func_node, void* private)
{
    ...
    entity->scope = resolver_proces_scope_current(process);
...
struct resolver_entity* resolver_get_entity_in_scope_with_entity_type(struct resolver_result* result, struct resolver_process* resolver, struct resolver_scope* scope, const char* entity_name, int entity_type)
{
    ...
    if (result && result->last_struct_union_entity)
    {
        ...
        {
            // Unions always start at offset 0
            offset = 0;
        }
        return resolver_make_entity(resolver, result, NULL, out_node, &(struct resolver_entity){.type=RESOLVER_ENTITY_TYPE_VARAIBLE, .offset=offset}, scope);
    }
...
// finds, for instance, a variable in an enclosing scope. keeps going up in scopes until it finds the name
...
{
    // gets first entity it finds with "entity_name"
...
{
    return resolver_get_entity_for_type(result, resolver, var_name, RESOLVER_ENTITY_TYPE_VARAIBLE);
}
...
struct resolver_entity* resolver_follow_for_name(struct resolver_process* resolver, const char* name, struct resolver_result* result)
{
    ...
    // functions that return structs, and variables that are structs
    if (entity->type == RESOLVER_ENTITY_TYPE_VARAIBLE && datatype_is_struct_or_union(&entity->var_data.dtype) ||
...
struct resolver_entity* resolver_follow_struct_expression(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    struct resolver_entity* result_entity = NULL;

    // if we have a.b, follow_part will resolve a and put it in the stack
    resolver_follow_part(resolver, node->exp.left, result);
    // we then capture c as left entity
    ...
    {
        // can't know offset of a pointer at compile time, i.e. a->b can't be merged
    ...
    // we may or may not need this in the future. revisit
    return NULL;
}
...
// We want to compute a stack size for the function call arguments. How many bytes do we need to use to complete the
// pushed of all the call arguments?
// Helps inform the code generator to build the stack
...
{
    // abc(50, 20, 30) -> 50, 20, 30 would be the argument node, combined together into an expression node
    ...
    else if (node_valid(argument_node))
    {
        ...
        // A variable was passed into the function call. Get the dtype size of that variable
        if (dtype)
        {
            // Will be 4 bytes unless it's a struct
...
struct resolver_entity* resolver_follow_parentheses(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    return resolver_follow_expression(resolver, node->parenthesis.exp, result);
}
...
{
    // e.g. char* abc; abc[3]. Datatype is not array. also applies to char arrays
...
struct resolver_entity* resolver_follow_arrary_bracket(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    // Reduce dtype. Similar to downcast
...
struct resolver_entity* resolver_follow_expression_parentheses(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
...
struct resolver_entity* resolver_follow_part_return_entity(struct resolver_process* resolver, struct node* node, struct resolver_result* result)
{
    ...
    switch (node->type)
    {
        ...
        case NODE_TYPE_EXPRESSION:
            entity = resolver_follow_expression(resolver, node, result);
            break;

        case NODE_TYPE_BRACKET:
            entity = resolver_follow_arrary_bracket(resolver, node, result);
            break;

        case NODE_TYPE_EXPRESSION_PARENTHESES:
            entity = resolver_follow_expression_parentheses(resolver, node, result);
            break;
