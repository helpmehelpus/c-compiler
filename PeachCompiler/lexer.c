...
bool op_valid(const char *op) {
    return S_EQ(op, "+") || S_EQ(op, "-") || S_EQ(op, "*") || S_EQ(op, "/") || S_EQ(op, "!") || S_EQ(op, "^") ||
           S_EQ(op, "+=") || S_EQ(op, "-=") || S_EQ(op, "*=") || S_EQ(op, "/=") || S_EQ(op, ">>") || S_EQ(op, "<<") ||
           S_EQ(op, ">=") || S_EQ(op, "<=") || S_EQ(op, ">") || S_EQ(op, "<") || S_EQ(op, "||") || S_EQ(op, "&&") ||
           S_EQ(op, "|") || S_EQ(op, "&") || S_EQ(op, "++") || S_EQ(op, "--") || S_EQ(op, "=") || S_EQ(op, "!=") ||
           S_EQ(op, "==") || S_EQ(op, "->") || S_EQ(op, "(") || S_EQ(op, "[") || S_EQ(op, ",") || S_EQ(op, ".") ||
...
static void lex_new_expression() {
    // Don't worry about memory management for now
...
static void lex_finish_expression() {
    ...
    if (lex_process->current_expression_count < 0) {
        compiler_error(lex_process->compiler, "Closed unopened expression\n");
    }
}
...
{
    // Skip x and read first hex digit
    nextc();
...
struct token *token_make_quote()
{
    ...
    if (nextc() != '\'')
    {
        compiler_error(lex_process->compiler, "Unclosed quote");
    }
...
struct token *read_next_token() {
    ...
    switch (c) {
        ...
        case EOF: // finished lexical analysis of file
            break;
...
struct lex_process* tokens_build_for_string(struct compile_process* compiler, const char* str)
{
    ...
    struct lex_process* _lex_process = lex_process_create(compiler, &lexer_string_buffer_functions, buffer);

    if ((!_lex_process) != LEXICAL_ANALYSIS_ALL_OK)
    {
        return NULL;
    }
