...
{
    struct node* function_node = node_create(&(struct node){.type=NODE_TYPE_FUNCTION, .func.name=name, .func.args.vector=arguments, .func.body_n=body_node, .func.rtype=*ret_type, .func.args.stack_addition=DATA_SIZE_DWORD});
...
{
    node_create(&(struct node){.type=NODE_TYPE_STATEMENT_SWITCH, .stmt.switch_statement.body=body_node, .stmt.switch_statement.exp=exp_node, .stmt.switch_statement.cases=cases, .stmt.switch_statement.has_default_case=has_default_case});
}
...
struct node* node_create(struct node* _node)
{
    ...
    // Allows us to know which body a particular node is in, and which function it's in
