...
struct parser_scope_entity
{
    ...
    // Every scope entity has a stack offset. Can be negative for local scope, positive for function argument scope
    int stack_offset;
...
static void expect_sym(char c)
{
    ...
    {
        compiler_error(current_process, "Expecting symbol %c but found different symbol\n", c);
    }
}
...
static void expect_op(const char* op)
{
    ...
    {
        compiler_error(current_process, "Expecting operator %s but found different operator\n", next_token->sval);
    }
}
...
void
parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token, struct token *datatype_secondary_token,
...
void parser_datatype_init(struct token *datatype_token, struct token *datatype_secondary_token,
                          struct datatype *datatype_out, int pointer_depth, int expected_type) {
    parser_datatype_init_type_and_size(datatype_token, datatype_secondary_token, datatype_out, pointer_depth,
                                       expected_type);
...
{
    parse_expressionable(history);
    // Future: do a literal evaluation at compile time, i.e. 10+20+30 = 60
...
void parser_scope_offset_for_stack(struct node* node, struct history* history)
{
    ...
    // The stack goes upwards when we look for function arguments, and downwards for local variables
    int offset = -variable_size(node);
    if (upward_stack)
    {
        #warning "HANDLE UPWARD STACK"
        compiler_error(current_process, "Upward stack handling not yet implemented\n");
    }
...
// Global variables do not offset to a stack. they only have a memory position
...
void make_variable_node_and_register(struct history* history, struct datatype* dtype, struct token* name_token, struct node* value_node)
{
    ...
    #warning "Remember to calculate scope offset and push to scope"
...
void make_variable_list_node(struct vector* var_list_vect)
{
    node_create(&(struct node){.type=NODE_TYPE_VARIABLE_LIST, .var_list=var_list_vect});
}
...
struct array_brackets* parse_array_brackets(struct history* history)
{
    ...
    {
        expect_op("[");
        // Reminder: left bracket is operator, right one is symbol
        if (token_is_symbol(token_peek_next(), ']'))
        {
            // empty brackets as in int x[] = {1, 2}
            expect_sym(']');
            break;
        }
...
void parse_variable(struct datatype* dtype, struct token* name_token, struct history* history)
{
    ...
    // We already parse things like int a;
...
void parser_append_size_for_node_struct_union(struct history* history, size_t* _variable_size, struct node* node)
{
    *_variable_size += variable_size(node);
    ...
    // in C, structures need to be aligned to the largest data type
...
// adjusts variable size based on the node that is provided
void parser_append_size_for_node(struct history* history, size_t* _variable_size, struct node* node)
{
    if (!node)
    {
...
void parser_finalise_body(struct history* history, struct node* body_node, struct vector* body_vec, size_t* _variable_size, struct node* largest_align_eligible_var_node, struct node* largest_possible_var_node)
{
    if (history->flags & HISTORY_FLAG_INSIDE_UNION)
    {
        // In C, union size is the largest possible variable inside a union (unions share memory)
    ...
    // Both structs and unions required padding
    int padding = compute_sum_padding(body_vec);
    *_variable_size += padding;

    // We are only interested in aligning for primitive types (structs will either be aligned or smaller)
...
void parse_variable_function_or_struct_union(struct history *history) {
    ...
    // int abc; "int" will be ignored and we will capture "abc"
    struct token* name_token = token_next();
    if (name_token->type != TOKEN_TYPE_IDENTIFIER)
    {
        compiler_error(current_process, "Invalid variable name in variable declaration\n");
    }

    // Check if this is a function declaration. Not yet implemented

    parse_variable(&dtype, name_token, history);

    // Parse multiple declarations in a line, e.g., int a, b, c, d = 20 (variable list)
    if (token_is_operator(token_peek_next(), ","))
    {
        struct vector* var_list = vector_create(sizeof(struct node*));
        // Pop off first variable
        struct node* var_node = node_pop();
...
void parse_keyword_for_global() {
    ...
    // Manipulate node before pushing it back
